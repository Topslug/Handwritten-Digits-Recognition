...
class TestConvNet:
    ...
    def gradient(self, x, t):
        ...
        # 결과 저장
        grads = {}
        for i, layer_idx in enumerate((0, 2, 4, 7, 10)):
...
class TestConvNet_Channel:
    ...
    def __init__(self, input_dim=(1, 28, 28),
        ...
        # 각 층의 뉴런 하나당 앞 층의 몇 개 뉴런과 연결되는가（TODO: 자동 계산되게 바꿀 것）
        pre_node_nums = np.array(
            [1 * 3 * 3,  16 * 3 * 3, 32 * 3 * 3, 64 * 14 * 14, hidden_size])
        wight_init_scales = np.sqrt(2.0 / pre_node_nums)  # ReLU를 사용할 때의 권장 초깃값
        ...
        # Affine 계층 가중치 초기화
        self.params['W4'] = wight_init_scales[3] * np.random.randn(64 * 14 * 14, hidden_size)
        self.params['b4'] = np.zeros(hidden_size)
        self.params['W5'] = wight_init_scales[4] * np.random.randn(hidden_size, output_size)
        self.params['b5'] = np.zeros(output_size)
